...
targets$color <- c(2,2,2,2,2,2,3,3,4,4,4,4,4,4,5,5,5)
...
targets
```
We will use the Expression Set as Raw Data
```{r}
#RAWDATA
if (!require(GEOquery)) {
...
```{r}
##DATA NORMALIZATION
eset1 <- log2(exprs(rawData))
eset <- ExpressionSet(eset1)
...
#BOXPLOT
boxplot(exprs(eset), las=2, main="Intensity distribution of Normalized data", cex.axis=0.6, 
...
write.csv2(eset_filtered$eset,"Filtered.csv2")
#NUMBER OF GENES OUT
...
We use these data frames to create volcano plots
```{r}
...
```
I am unable to identify the genes as the annotation is not in bioconductor 
